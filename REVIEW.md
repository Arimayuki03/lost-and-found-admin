...
## 七、第二轮：运行时验证与后续修复（2026-09-14）

首轮修复完成后，启动了真实环境（Flask 后端 + MySQL/Redis/MinIO + Vite Dev）进行浏览器冒烟测试，并顺手修复了测试中发现的问题。

### 7.1 冒烟测试结果

| 场景 | 结果 |
|------|------|
| 错误密码登录 | ✅ 返回 401、**无页面重载**、正确显示「登录失败：用户名或密码错误」（3 次运行均通过，后端日志确认请求到达） |
| 正确密码登录 | ✅ 进入管理员控制面板，统计卡片与 ECharts 图表正常渲染 |
| 用户管理列表 | ✅ 改走 API 封装层后的 `GET /admin/users` 正常返回并渲染 |
| token 刷新三步流程 | ✅ 坏 access token → 401/422 → `POST /common/refresh` 换新 → 重试成功 |
| 编辑用户 | ✅ `PUT /admin/users/:id` 生效，成功提示 + 列表刷新 |

### 7.2 测试中发现并修复的新问题

1. **JWT 422 未纳入刷新流程**（P2，已修复并实测）：flask-jwt-extended 对**格式损坏**的 token 返回 422 而非 401，原拦截器只处理 401，导致 localStorage 中 token 损坏后页面永久卡死（所有请求失败且无法自愈）。现 401/422 统一进入刷新分支，实测损坏 token 页面可自动恢复。
2. **上传接口无认证**（安全，已修复）：后端 `POST /common/images/upload` 原本无任何鉴权，任何人可向 MinIO 上传文件。已加 `@jwt_required()`（用户端 uni-app 上传本就携带 token，不受影响；管理端 3 处 el-upload 已补充 `:headers` 认证头）。回归验证：无 token → 401，带 token → 200。
3. **管理员用户列表 N+1 查询**（性能，已修复）：`GET /admin/users` 原对每个用户执行 2 次 COUNT，改为两次 `GROUP BY` 聚合查询（user 端 sadmin 列表无此问题）。

### 7.3 运维注意事项

- 修复验证过程中发现 **端口 5000 被两个后端进程同时监听**（本会话与另一开发会话各启动了一个），请求被随机分流，导致鉴权修复一度被旧实例旁路。现已收敛为单实例。**启动后端前请确认 5000 端口未被占用**，否则新旧代码混流会出现难以排查的随机行为。
...
4. ~~后端顺带发现：`/common/images/upload` 无认证可任意上传；管理员用户列表 N+1 查询~~ **已于 2026-09-14 第二轮修复**（见 7.2）。
